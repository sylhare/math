...
def _(go):
    ...
    _years = [e[0] for e in _events]
    _labels = [e[1] for e in _events]
    _sides = [e[2] for e in _events]
    ...
    _fig.add_trace(go.Scatter(
        x=[1820, 2005], y=[0, 0],
        mode='lines', line={'color': '#a0a0a0', 'width': 2},
        showlegend=False, hoverinfo='skip',
    ))

    # Tick marks and stems
    for _yr, _lbl, _side in _events:
        _color = '#00d4ff' if _side == 1 else '#ff6b6b'
        _fig.add_trace(go.Scatter(
            x=[_yr, _yr], y=[0, _side * 0.55],
            mode='lines', line={'color': _color, 'width': 2},
            showlegend=False, hoverinfo='skip',
        ))
        _fig.add_trace(go.Scatter(
            x=[_yr], y=[0],
            mode='markers', marker={'color': _color, 'size': 10},
            showlegend=False,
            hovertemplate=f'{_yr}: {_lbl.replace(chr(10), " ")}<extra></extra>',
        ))
        _fig.add_annotation(
            x=_yr, y=_side * 0.65,
            text=f'<b>{_yr}</b><br>{_lbl}',
    ...
    _fig.update_layout(
        ...
        xaxis={
            'gridcolor': '#2d3a4f', 'zerolinecolor': '#2d3a4f',
            'range': [1815, 2008], 'title': 'Year',
            'tickvals': _years,
        },
...
def _(go, make_subplots, np):
    # Draw the unknot, trefoil, and figure-8 knot diagrams side by side
    ...
    def _draw_figure8():
        t = np.linspace(0, 2 * np.pi, 500)
        x = (2 + np.cos(2 * t)) * np.cos(3 * t) / 4
        y = (2 + np.cos(2 * t)) * np.sin(3 * t) / 4
        return x, y

    _fig = make_subplots(
        rows=1, cols=3,
        subplot_titles=["The Unknot (0 crossings)", "Trefoil Knot (3 crossings)", "Figure-8 Knot (4 crossings)"],
        horizontal_spacing=0.05,
    )

    # Unknot
    _ux, _uy = _draw_unknot()
    _fig.add_trace(go.Scatter(
        x=_ux, y=_uy,
        mode='lines',
    ...
    _fig.update_layout(
        ...
        plot_bgcolor='#16213e',
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
        title='Classic Knots (Parametric Projections)',
        height=350,
    )
    ...
    for _col in range(1, 4):
        _fig.update_xaxes(
            ...
            showticklabels=False, scaleanchor=f'y{_col if _col > 1 else ""}',
            row=1, col=_col,
        )
        _fig.update_yaxes(
            gridcolor='#2d3a4f', zerolinecolor='#a0a0a0',
    ...
    for _ann in _fig['layout']['annotations']:
        _ann['font'] = {'color': '#ffe66d', 'size': 13}
...
@app.cell
def _(go, make_subplots, np):
    # Visualize positive and negative crossings
    _fig = make_subplots(
        rows=1, cols=2,
    ...
    _fig.update_layout(
        ...
        plot_bgcolor='#16213e',
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
        title='Crossing Signs (Vertices in Knot Diagrams)',
        height=350,
    )
    for _c in range(1, 3):
        _fig.update_xaxes(
            range=[-1.3, 1.3], showticklabels=False,
            gridcolor='#2d3a4f', zerolinecolor='#2d3a4f',
            row=1, col=_c,
        )
        _fig.update_yaxes(
...
@app.cell
def _(go, make_subplots, np):
    # Visualize the three Reidemeister moves schematically
    ...
    _fig.add_trace(go.Scatter(
        x=[-0.8, 0.8], y=[-0.8, 0.8],
        mode='lines',
        line={'color': '#4ecdc4', 'width': 3, 'dash': 'dot'},
        showlegend=False,
    ), row=1, col=1)
    ...
    _fig.add_trace(go.Scatter(
        x=[-0.9, 0.9], y=[-0.6, -0.6],
        mode='lines',
        line={'color': '#00d4ff', 'width': 3, 'dash': 'dot'},
        showlegend=False,
    ), row=1, col=2)
    _fig.add_trace(go.Scatter(
        x=[-0.9, 0.9], y=[-0.9, -0.9],
        mode='lines',
        line={'color': '#ff6b6b', 'width': 3, 'dash': 'dot'},
        showlegend=False,
    ), row=1, col=2)
    ...
    _fig.update_layout(
        ...
        plot_bgcolor='#16213e',
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
        title='The Three Reidemeister Moves (solid ↔ dashed)',
        height=380,
    )
    for _c in range(1, 4):
        _fig.update_xaxes(
            range=[-1.1, 1.1], showticklabels=False,
            gridcolor='#2d3a4f', zerolinecolor='#2d3a4f',
            row=1, col=_c,
        )
        _fig.update_yaxes(
...
@app.cell
def _(go, make_subplots, np):
    # Visualize key knot invariants as a comparison table/bar chart
    ...
    _colors_cn = ['#4ecdc4', '#00d4ff', '#00d4ff', '#00d4ff', '#00d4ff']
    _colors_det = ['#4ecdc4', '#ff6b6b', '#ff6b6b', '#ffe66d', '#ff6b6b']
    _colors_un = ['#4ecdc4', '#95e1d3', '#95e1d3', '#ffe66d', '#95e1d3']
    ...
    _fig.update_layout(
        ...
        plot_bgcolor='#16213e',
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
        title='Comparing Knot Invariants for Classic Knots',
        height=380,
    )
    for _c in range(1, 4):
        _fig.update_xaxes(gridcolor='#2d3a4f', row=1, col=_c)
        _fig.update_yaxes(gridcolor='#2d3a4f', row=1, col=_c)
    for _ann in _fig['layout']['annotations']:
        _ann['font'] = {'color': '#ffe66d', 'size': 13}
...
@app.cell
def _(go, np):
    # Arc diagram: trefoil with the three arcs and three vertices labeled
    _t_full = np.linspace(0, 2 * np.pi, 600)
    ...
    _arc_colors = ['#ff6b6b', '#00d4ff', '#4ecdc4']
    ...
    for _i in range(3):
        ...
        _sx, _sy = _txy(_seg_t)
        _mx, _my = _txy(np.array([(_t_start + _t_end) / 2]))
        _fig.add_trace(go.Scatter(
    ...
    for _i, _ct in enumerate(_cross_t):
        _vx, _vy = _txy(np.array([_ct]))
        _fig.add_trace(go.Scatter(
            x=[float(_vx[0])], y=[float(_vy[0])],
            mode='markers',
            marker={'color': '#ffe66d', 'size': 16, 'symbol': 'circle'},
            showlegend=_i == 0,
...
@app.cell
def _(arc1_color, arc2_color, arc3_color, go, np):
    ...
    _x1, _y1 = _trefoil_xy(_t1)
    _x2, _y2 = _trefoil_xy(_t2)
    _x3, _y3 = _trefoil_xy(_t3)
    ...
    _status_color = '#4ecdc4' if _is_tricolorable else '#ff6b6b'
...
@app.cell
def _(go, np):
    # Visualize Alexander polynomial values at t=-1 giving the determinant
    ...
    for _f, _label, _color, _det in [
        (_alex_unknot, 'det=1', '#4ecdc4', 1),
        (_alex_trefoil, 'det=3', '#ff6b6b', 3),
        (_alex_figure8, 'det=5', '#00d4ff', 5),
    ]:
    ...
    _fig.add_vline(x=-1, line_color='#ffe66d', line_width=1, line_dash='dash')
    _fig.add_annotation(
        x=-1, y=6,
    ...
    _fig.update_layout(
        ...
        xaxis={
            'gridcolor': '#2d3a4f', 'zerolinecolor': '#a0a0a0', 'title': 't',
            'range': [-2, 2],
        },
        yaxis={
            'gridcolor': '#2d3a4f', 'zerolinecolor': '#a0a0a0',
...
@app.cell
def _(go, make_subplots, np):
    # Skein relation: K+, K-, K0 side by side
    _fig = make_subplots(
        rows=1, cols=3,
    ...
    _fig.update_layout(
        ...
        plot_bgcolor='#16213e',
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
        title='Skein Triple: the three local diagrams related by the skein relation',
        height=320,
    )
    for _c in range(1, 4):
        _fig.update_xaxes(range=[-1.3, 1.3], showticklabels=False,
            gridcolor='#2d3a4f', zerolinecolor='#2d3a4f', row=1, col=_c)
        _fig.update_yaxes(range=[-1.3, 1.3], showticklabels=False,
            gridcolor='#2d3a4f', zerolinecolor='#2d3a4f', row=1, col=_c)
    for _ann in _fig['layout']['annotations']:
        _ann['font'] = {'color': '#ffe66d', 'size': 12}
    _fig
    return
...
def _(go, knot_selector, np):
    ...
    _fig.update_layout(
        ...
        scene={
            'bgcolor': '#16213e',
            'xaxis': {'gridcolor': '#2d3a4f', 'color': '#a0a0a0', 'showticklabels': False},
            'yaxis': {'gridcolor': '#2d3a4f', 'color': '#a0a0a0', 'showticklabels': False},
            'zaxis': {'gridcolor': '#2d3a4f', 'color': '#a0a0a0', 'showticklabels': False},
        },
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
...
def _(mo):
    mo.md(
        ...
        **Torus knots** all live on the surface of a torus (donut shape). The torus knot
        ...
        circle. Two torus knots $T(p,q)$ and $T(q,p)$ are equivalent (mirror images
        of each other when $p, q$ are not coprime), but $T(p,q) \neq T(p', q')$
        unless $\{p,q\} = \{p', q'\}$.

        **Torus knot invariants:**
        - Crossing number: $c(T(2,n)) = n-1$ for odd $n$
...
@app.cell
def _(go, make_subplots, np):
    # Visualize the Hopf link and Borromean rings (2D projections)
    ...
    _bor_colors = ['#00d4ff', '#ff6b6b', '#4ecdc4']
    ...
    _fig.update_layout(
        ...
        plot_bgcolor='#16213e',
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
        title='Links: Hopf Link and Borromean Rings',
        height=380,
        legend={'bgcolor': 'rgba(22, 33, 62, 0.8)'},
    )
    for _c in range(1, 3):
...
@app.cell
def _(go, np):
    # Visualize DNA linking number concept
    ...
    _fig.update_layout(
        ...
        scene={
            'bgcolor': '#16213e',
            'xaxis': {'gridcolor': '#2d3a4f', 'color': '#a0a0a0'},
            'yaxis': {'gridcolor': '#2d3a4f', 'color': '#a0a0a0'},
            'zaxis': {'gridcolor': '#2d3a4f', 'color': '#a0a0a0', 'title': 'Position along DNA'},
        },
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
...
def _(mo):
    mo.md(
        r"""
        The DNA double helix has a linking number of approximately 10 base pairs per
        full turn. For a relaxed DNA circle with $N$ base pairs, the linking number
        is $\text{Lk} \approx N/10$.
