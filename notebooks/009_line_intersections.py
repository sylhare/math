...
def _(go):
    ...
    for _yr, _lbl, _side in _events:
        ...
        _fig.add_annotation(
            x=_yr, y=_side * 0.65,
            text=f'<b>{_yr if _yr > 0 else str(abs(_yr)) + " BCE"}</b><br>{_lbl}',
    ...
    _fig.update_layout(
        ...
        xaxis={
            ...
            'range': [-500, 2100], 'title': 'Year',
            'tickvals': _years,
            'ticktext': [str(abs(y)) + ' BCE' if y < 0 else str(y) for y in _years],
        },
...
@app.cell
def _(mo):
    mo.md(r"### Interactive: Three Representations of the Same Line")
    return
...
@app.cell
def _(go, line_intercept, line_slope, np):
    _m = line_slope.value
    ...
    _fig.update_layout(
        ...
        legend={'bgcolor': 'rgba(22, 33, 62, 0.8)', 'font': {'size': 11}},
        height=480,
    )
    _fig
    return
...
def _(go, make_subplots, np):
    # Diagram: the three cases side by side
    _fig = make_subplots(
        rows=1, cols=3,
        subplot_titles=[
            "D ≠ 0  →  Intersecting",
    ...
    _fig.update_layout(
        ...
        plot_bgcolor='#16213e',
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
        title='The Three Cases — All Detected by the Determinant D = a₁b₂ − a₂b₁',
        height=350,
    )
    for _c in range(1, 4):
        _fig.update_xaxes(range=[-3, 3], showticklabels=False, gridcolor='#2d3a4f', zerolinecolor='#a0a0a0', row=1, col=_c)
        _fig.update_yaxes(range=[-3, 3], showticklabels=False, gridcolor='#2d3a4f', zerolinecolor='#2d3a4f', scaleanchor=f'x{_c}', row=1, col=_c)
...
@app.cell
def _(mo):
    mo.md(r"### Interactive Line Intersection")
    return
...
@app.cell
def _(go, l1_intercept, l1_slope, l2_intercept, l2_slope, np):
...
@app.cell
def _(go, np):
    # Visualize the determinant as a parallelogram area
...
@app.cell
def _(mo):
    mo.md(r"### Interactive: Parametric Intersection")
    return
...
@app.cell
def _(d1x, d1y, d2x, d2y, go, np, p1x, p1y, p2x, p2y):
...
def _(mo):
    mo.md(
        ...
        which is the **cross product** $\mathbf{d}_1 \times \mathbf{d}_2$ (the $z$-component).
        """
    )
    return
...
@app.cell
def _(go, np):
    # Visualize orientation (cross product sign) for three points

    _A = np.array([0.0, 0.0])
    _B = np.array([3.0, 1.0])
    _C_ccw = np.array([2.0, 3.0])   # Left turn
    _C_cw = np.array([2.0, -1.5])   # Right turn

    def _orientation_fig(A, B, C, title, color):
        cross = (B[0] - A[0]) * (C[1] - A[1]) - (B[1] - A[1]) * (C[0] - A[0])
        sign = "+" if cross > 0 else ("-" if cross < 0 else "0")
        return A, B, C, cross, sign
    ...
    _fig.update_layout(
        ...
        plot_bgcolor='#16213e',
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
        title='Orientation: Sign of the Cross Product',
        height=380,
    )
    for _c in range(1, 3):
        _fig.update_xaxes(gridcolor='#2d3a4f', zerolinecolor='#a0a0a0', range=[-0.5, 4], showticklabels=False, row=1, col=_c)
        _fig.update_yaxes(gridcolor='#2d3a4f', zerolinecolor='#a0a0a0', range=[-2, 4], showticklabels=False, row=1, col=_c)
    for _ann in _fig['layout']['annotations']:
        _ann['font'] = {'color': '#ffe66d', 'size': 12}
...
def _(go, np, skew_t):
    ...
    _fig.update_layout(
        ...
        scene={
            'bgcolor': '#16213e',
            'xaxis': {'gridcolor': '#2d3a4f', 'color': '#a0a0a0', 'title': 'x'},
            'yaxis': {'gridcolor': '#2d3a4f', 'color': '#a0a0a0', 'title': 'y'},
            'zaxis': {'gridcolor': '#2d3a4f', 'color': '#a0a0a0', 'title': 'z'},
        },
        font={'color': '#eaeaea', 'family': 'JetBrains Mono, monospace'},
        title=_status_3d,
        legend={'bgcolor': 'rgba(22, 33, 62, 0.8)'},
...
@app.cell
def _(go, np):
    # Visualize a planar graph with vertices at line intersections
    # Draw the edges of a cube projected onto the plane (planar graph)
