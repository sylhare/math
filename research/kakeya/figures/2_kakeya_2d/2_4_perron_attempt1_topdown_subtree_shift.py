...
def sprout(a, b, apex, depth, overlap):
    ...
    if depth == 0:
        return [tri(a, b, apex)]
    m = 0.5 * (a + b)
    left = sprout(a, m, apex, depth - 1, overlap)
    right = sprout(m, b, apex, depth - 1, overlap)
    delta = overlap * (m - a)
...
def covered_directions(polys, n=360):
    ...
    for k in range(n):
        ...
        # proxy: does the union contain a length-L segment in direction th?
        ok = False
        minx, miny, maxx, maxy = u.bounds
        for _ in range(40):
            px = np.random.uniform(minx, maxx)
            py = np.random.uniform(miny, maxy)
            seg = _seg(px, py, d, 0.25)  # modest chord, not full unit
            if u.contains(seg):
                ok = True
                break
        hits += ok
...
# recenter on base midpoint so copies share a pivot
pivot = (0.5, 0.0)
