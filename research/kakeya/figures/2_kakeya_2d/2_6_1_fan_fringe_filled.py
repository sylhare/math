...
def build():
    ...
    # corner fans: bases slide onto the two edges so the lobe merges with the core
    for i, d0 in enumerate(CORNERS_DEG):
        v = VERTS[i]
        left = VERTS[(i - 1) % 3]  # neighbour for +offset half
        right = VERTS[(i + 1) % 3]  # neighbour for -offset half
        for th in np.linspace(d0 - FAN_HALF, d0 + FAN_HALF, J_CORNER):
            off = (th - d0) / FAN_HALF  # -1..+1 across the fan
            nb = left if off >= 0 else right
            base = v + BASE_SPREAD * abs(off) * (nb - v)
            taper = max(0.0, math.cos(math.radians((th - d0) * 1.35))) ** 1.4
            ragged = 0.45 + 0.75 * rng.random() ** 1.6
            length = LEN_CORNER * (0.18 + 0.82 * taper) * ragged
            needles.append(_sliver(base, _unit(th), length))

    # edge fringe: outward-normal needles, longest near the corners
    for a, b in ((VERTS[0], VERTS[1]), (VERTS[1], VERTS[2]), (VERTS[2], VERTS[0])):
        ...
        for t in np.linspace(0.06, 0.94, K_EDGE):
            length = LEN_EDGE * (0.55 + 0.85 * rng.random())
