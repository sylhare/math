...
def perron(n, alpha):
    ...
    for _ in range(n):
        ...
        shapes = [
            [translate(p, xoff=+step) for p in a] + [translate(p, xoff=-step) for p in b]
...
def build():
    ...
    tips = [
        cen + (H - cen[1]) * np.array([math.cos(math.radians(d)), math.sin(math.radians(d))])
        for d in (90.0, 210.0, 330.0)
    ]  # the three rotated apexes
    off = fan_offsets(N)
    needles = []
    for i, d0 in enumerate((90.0, 210.0, 330.0)):
        tip = tips[i]
        for o in off:
            neigh = tips[(i - 1) % 3] if o >= 0 else tips[(i + 1) % 3]
            base = tip + BASE_SPREAD * abs(o) / FAN_HALF * (neigh - tip)
    ...
    for a, b in ((tips[0], tips[1]), (tips[1], tips[2]), (tips[2], tips[0])):
        ...
        for t in np.linspace(0.08, 0.92, K_EDGE):  # edge fringe
