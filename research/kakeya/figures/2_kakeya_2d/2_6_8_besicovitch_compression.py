...
FAN_HALF = 75.0  # half-opening of each corner fan (150 deg)
M = 58  # needles per fan
LEN = 1.0  # needle length (unit)
DUTY = 0.36  # drawn angular width / delta spacing
K_EDGE = 22  # outward-normal needles per edge
LEN_EDGE = 0.34
YELLOW = "#f4e37a"
...
def build():
    ...
    needles = []
    for i, radial in enumerate(CORNERS_DEG):
        apex = VERTS[i]
...
def main():
    ...
    dirs = np.sort(
        np.concatenate(
            [  # directions carried by the three corner fans
    ...
    math_check(
        ...
        [
            ...
            (
                "coverage",
                f"3 overlapping fans x {2 * FAN_HALF:.0f} deg cover 360 deg; max direction gap {max_gap:.1f} deg",
            ),
