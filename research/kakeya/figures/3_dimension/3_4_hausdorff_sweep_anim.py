...
def main():
    ...
    def draw_left(m, s):
        ...
        for a, length in dust:
            ax[0].plot([a + length / 2.0], [0.52], marker="|", ms=9, color=COLORS["needle"], mew=1.1, zorder=3)
        ax[0].text(
            0.5,
            0.80,
            f"Cantor cover: 2^{m} = {2**m} intervals of 3^-{m}",
            ha="center",
            va="center",
            fontsize=10,
            color=COLORS["guide"],
        ...
        ax[0].text(
            0.5,
            0.24,
            f"level-{DUST_LEVEL} dust shown; sum = (2*3^-s)^m",
            ha="center",
            va="center",
            fontsize=8.5,
            color=COLORS["muted"],
        ...
        if h >= 0:
            ax[0].add_patch(
                mpatches.Rectangle((GX0, GY), GX1 - GX0, h, facecolor=gcol, edgecolor="none", alpha=0.9, zorder=2)
            )
        else:
            ax[0].add_patch(
                mpatches.Rectangle((GX0, GY + h), GX1 - GX0, -h, facecolor=gcol, edgecolor="none", alpha=0.9, zorder=2)
            )
        ax[0].plot([GX0 - 0.01, GX1 + 0.01], [GY, GY], color=COLORS["guide"], lw=1.0, ls="--", zorder=3)
        ax[0].text(GX1 + 0.015, GY, "=1", va="center", fontsize=8, color=COLORS["guide"])
        state = "huge" if lg > 0.02 else ("tiny" if lg < -0.02 else "= 1")
        ax[0].text((GX0 + GX1) / 2.0, GY + GH + 0.06, f"sum = {val:.3f}", ha="center", fontsize=9, color=gcol)
        ax[0].text((GX0 + GX1) / 2.0, GY - GH - 0.06, state, ha="center", va="top", fontsize=8.5, color=gcol)
    ...
    def draw_right(kind, s, m_cur):
        ...
        ax[1].text(DIM + 0.01, Y_TOP - 0.35, f"s = dim_H = {DIM:.4f}", color=COLORS["guide"], fontsize=9)

        if kind == "sweep":
            yv = np.clip([hausdorff_sum(sv, PHASE1_M) for sv in s_plot], 0, Y_TOP)
            ax[1].plot(s_plot, yv, color=COLORS["needle"], lw=2.0, label=f"m = {PHASE1_M}")
            cur = hausdorff_sum(s, PHASE1_M)
            ax[1].plot([s], [min(cur, Y_TOP)], "o", color=COLORS["accent"], ms=11, mfc="none", mew=2, zorder=7)
            ax[1].plot([s, s], [0, min(cur, Y_TOP)], color=COLORS["accent"], lw=0.8, ls=":", zorder=5)
            ax[1].set_title(f"phase 1: sweep s at fixed depth m = {PHASE1_M}", fontsize=11)
            ...
            for m in range(1, m_cur + 1):
                ...
                ax[1].text(
                    S_LO + 0.005,
                    min(hausdorff_sum(S_LO, m), Y_TOP - 0.15),
                    f"m={m}",
                    fontsize=8,
                    color=COLORS["accent"] if is_cur else COLORS["needle"],
                    va="top",
                )
            ax[1].set_title(f"phase 2: depths m = 1..{m_cur} accumulate (steepen to a step)", fontsize=11)
