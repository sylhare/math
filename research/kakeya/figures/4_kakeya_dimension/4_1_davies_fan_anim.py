...
def main():
    ...
    for a in angles:
        ...
        if a == 0.0:
            measured.append(math.nan)
            formula.append(math.nan)
        else:
            measured.append(base.intersection(polys[-1]).area)
            formula.append(DELTA**2 / math.sin(a))

    rel_errs = [abs(measured[k] - formula[k]) / formula[k] for k in range(1, N_DIR)]
    ...
    rows = [
        (
            f"theta={math.degrees(angles[k]):5.1f} deg",
            f"measured {measured[k]:.3e}  formula {formula[k]:.3e}  "
            f"err {abs(measured[k] - formula[k]) / formula[k] * 100:.2f}%",
        )
        for k in sample
    ]
    ...
    def update(fi):
        ...
        ax[0].set_xlim(-0.62, 0.62)
        ax[0].set_ylim(-0.62, 0.62)
        ax[0].set_title(f"fan of {k + 1} directions through a center\nunion area {unions[k]:.4f}  (grows with spread)")

        ax[1].cla()
        ax[1].plot(np.degrees(th_dense), ov_dense, color=COLORS["muted"], lw=1.2, label="delta^2 / sin theta")
        km = [j for j in range(1, k + 1)]
        if km:
            ax[1].plot(
                [math.degrees(angles[j]) for j in km],
                [measured[j] for j in km],
                "o",
                color=COLORS["accent"],
                ms=5,
        ...
        if k >= 1:
            ax[1].plot(math.degrees(angles[k]), measured[k], "o", color=COLORS["accent"], ms=11, mfc="none", mew=2)
        ax[1].set_xlim(0, 180)
        ax[1].set_ylim(0, DELTA**2 / math.sin(math.radians(8)))
        ax[1].set_xlabel("theta (deg)")
        ax[1].set_ylabel("pairwise overlap area")
