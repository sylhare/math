"""Animation: the Davies overlap engine, building a fan of rectangles (section 4 of ../kakeya.md).

Mirrors davies_rectangles.py in motion. A direction theta sweeps across [0, pi); at each step a
1 x delta rectangle in that direction is added to a fan through a common center. Two live readouts:

  * overlap of the new rectangle (angle theta) with the base rectangle (angle 0),

        | R_theta cap R_0 |  =  delta^2 / sin theta        (two strips of width delta at angle theta);

    the right panel plots this formula curve and drops the MEASURED (shapely) overlap on top of it.
  * the growing union area of the whole fan, shown on the left panel and only ever increasing.

Small overlaps in separated directions is exactly what forces the union to spread out: this is the
"area 0 but dimension 2" mechanism. Geometric honesty: overlaps are MEASURED by intersecting the
drawn rectangles, not asserted, and checked against delta^2/sin theta; the union is monotone.
...
DELTA = 0.05           # drawn / measured rectangle width
N_DIR = 36             # directions across [0, pi)
END_HOLD = 8


# --- geometry: pure-numpy rectangle (mirrors the static figure) ---------------------------
...
def main():
    ...
    # precompute measured overlap vs formula, and the running union area
    ...
    # invariant 1: measured overlap matches delta^2/sin theta
    ...
    # invariant 2: union area is non-decreasing as directions spread
    ...
    def update(fi):
        ...
        # right: overlap vs theta, formula curve + measured points so far
        ax[1].cla()
