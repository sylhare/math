...
DELTA = 0.06  # tube radius (half-width)
BLOB_C = (0.22, 0.14)  # centre of indicator disc
BLOB_R = 0.12
GRID_N = 160
EXTENT = 0.62
...
def main():
    ...
    oi = int(np.argmin(np.abs(xs)))  # grid index nearest origin
    ...
    for a in fan:
        ...
        ax[0].add_patch(
            plt.Polygon(
                corners, closed=True, facecolor=COLORS["needle"], edgecolor=COLORS["needle"], alpha=0.35, lw=0.8
            )
        )
    th = np.linspace(0, 2 * np.pi, 200)
    ax[0].plot(BLOB_C[0] + BLOB_R * np.cos(th), BLOB_C[1] + BLOB_R * np.sin(th), color=COLORS["accent"], lw=1.4)
    ...
    im = ax[1].imshow(
        np.ma.masked_where(Mf <= 0, Mf),
        origin="lower",
        extent=(-EXTENT, EXTENT, -EXTENT, EXTENT),
        cmap="magma",
        vmin=0.0,
    )
    ax[1].plot(BLOB_C[0] + BLOB_R * np.cos(th), BLOB_C[1] + BLOB_R * np.sin(th), color="white", lw=1.2, alpha=0.9)
    ...
    fig.suptitle(
        "Kakeya maximal function:  || f*_delta ||_{L^n(S^{n-1})} <= C_eps delta^{-eps} || f ||_{L^n(R^n)}   (n = 2, 3)",
        fontsize=11,
    )
