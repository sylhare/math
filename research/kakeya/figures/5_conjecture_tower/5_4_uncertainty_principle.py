...
def main():
    r, s = 4.0, 1.5  # frequency-box sides
    fr, fs = 1.0 / r, 1.0 / s  # dual physical-box sides
    ...
    ax[0].add_patch(
        Rectangle((-r / 2, -s / 2), r, s, facecolor=COLORS["outer"], alpha=0.35, edgecolor=COLORS["outer"], lw=1.6)
    )
    ...
    ax[1].add_patch(
        Rectangle(
            (-fr / 2, -fs / 2), fr, fs, facecolor=COLORS["accent"], alpha=0.35, edgecolor=COLORS["accent"], lw=1.6
        )
    )
