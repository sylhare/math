...
def freq_rectangle(phi: float, r: float, r2: float) -> np.ndarray:
    ...
    t = np.array([-math.sin(phi), math.cos(phi)])  # tangential unit
    p = n  # touch point on unit circle
    return np.array(
        [
            p - (r / 2) * t,
            p + (r / 2) * t,
...
def dist_center_to_inner_edge(rect: np.ndarray) -> float:
    ...
    normal = np.array([-d[1], d[0]])  # unit normal to edge
...
def main():
    ...
    # opposite angles share a tube -> 6 orientations
    ...
    math_check(
        ...
        [
            ...
            (
                "phys aspect  1/r^2 : 1/r",
                f"{1 / r2:.4f} : {1 / r:.4f}  = {(1 / r2) / (1 / r):.4f} : 1  (want 1/r = {1 / r:.4f})",
            ),
    ...
    ax[0].plot(
        np.append(disc[:, 0], disc[0, 0]), np.append(disc[:, 1], disc[0, 1]), color=COLORS["accent"], lw=2.2, zorder=3
    )
    ...
    top = freq_rects[3]  # slab near top (phi=90 for N=12)
    a, b = top[0], top[1]
    mid = (a + b) / 2
    ax[0].annotate("", xy=tuple(b), xytext=tuple(a), arrowprops=dict(arrowstyle="<->", color=COLORS["guide"], lw=1.0))
    ax[0].text(mid[0], mid[1] + 0.12, r"$r$", ha="center", va="bottom", fontsize=14)
    c, d = top[0], top[3]  # right short edge (thickness r^2)
    ax[0].annotate("", xy=tuple(d), xytext=tuple(c), arrowprops=dict(arrowstyle="<->", color=COLORS["guide"], lw=1.0))
    ...
    bl, br = vtube[3], vtube[0]  # short (width 1/r) edge
    ax[1].annotate("", xy=tuple(br), xytext=tuple(bl), arrowprops=dict(arrowstyle="<->", color=COLORS["guide"], lw=1.0))
    ax[1].text((bl[0] + br[0]) / 2, bl[1] - 0.18, r"$1/r$", ha="center", va="top", fontsize=13)
    ax[1].annotate(
        "",
        xy=(lim - 0.12, 0.5 / r2),
        xytext=(lim - 0.12, -0.5 / r2),
