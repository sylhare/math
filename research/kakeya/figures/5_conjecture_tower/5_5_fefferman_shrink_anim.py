...
def freq_rectangle(phi: float, r: float, r2: float) -> np.ndarray:
    ...
    p = n
    return np.array([p - (r / 2) * t, p + (r / 2) * t, p + (r / 2) * t + r2 * n, p - (r / 2) * t + r2 * n])
...
def main():
    ...
    # largest slab (N_MIN) so nothing clipped
    _fext = 0.0
    for _n in ns:
        _r = 2.0 * math.sin(math.pi / int(_n))
        _r2 = _r * _r
    ...
    def update(i):
        ...
        for phi in np.linspace(0.0, 2 * math.pi, n_rect, endpoint=False):
            ...
            artists.extend([p, e])
        # normalise to unit length (length 1, width r)
