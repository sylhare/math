...
# top (strongest) to bottom (weakest)
TOWER = [
    (
        "Local smoothing",
        "Sogge",
...
def main():
    ...
    gap = 0.72
    top_y = 9.35
    # widen toward base (floor widest)
    widths = [5.6, 6.2, 6.8, 7.6]
    fills = [COLORS["region"], COLORS["needle"], COLORS["outer"], COLORS["accent"]]
    text_colors = ["black", "white", "white", "white"]

    centers_y = []
    for i, ((name, who, formula, status), w, fc, tc) in enumerate(zip(TOWER, widths, fills, text_colors, strict=False)):
        y = top_y - i * (box_h + gap)
        cy = y - box_h / 2
        centers_y.append(cy)
        box = FancyBboxPatch(
        ...
        ax.add_patch(box)
        ax.text(5, cy + 0.42, name, ha="center", va="center", fontsize=13, fontweight="bold", color=tc)
        ...
        ax.text(
            5, y - box_h - 0.30, status, ha="center", va="center", fontsize=8, color=COLORS["guide"], style="italic"
        )
    ...
    for i in range(n - 1):
        ...
        ax.add_patch(arr)
        ax.text(
            5.35, (y_from + y_to) / 2 - 0.14, "implies", ha="left", va="center", fontsize=8.5, color=COLORS["guide"]
        )

    ax.text(
        5,
        9.75,
        "Harmonic-analysis tower  (implications run downward)",
        ha="center",
        va="center",
        fontsize=12,
        fontweight="bold",
    ...
    ax.text(
        5,
        0.28,
        "Kakeya is the geometric floor: implied by all above, it implies none of them.",
        ha="center",
        va="center",
        fontsize=8.5,
        color=COLORS["guide"],
    ...
    strongest_top = order[0] == "Local"  # strongest / at top
    no_false_claim = kakeya_at_base  # Kakeya is a sink
