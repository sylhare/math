...
DELTAS = [0.2, 0.1, 0.05, 0.025]  # delta ladder, halved each step
N_SAMPLES = 90000  # enough that delta=0.025 not truncated
EPS = 0.05  # dim-3 floor |N_delta| >= c_eps delta^eps
DRAWN = [14, 26, 42, 64]  # tubes drawn per stage
STAGE_HOLD = 9  # frames per stage
END_HOLD = 8  # trailing hold frames
...
def main():
    ...
    # #directions = chord-delta packing on S^2, ~4x per halving
    samples = fibonacci_sphere(N_SAMPLES)
    counts = np.array([len(delta_separated(samples, d)) for d in DELTAS], dtype=float)

    # |T| = delta^2; content = #T*|T| ~ const
    tube_vol = deltas**2
    content = counts * tube_vol
    content_norm = content / content[0]

    growth = counts[1:] / counts[:-1]  # ~4 per halving
    vol_ratio = tube_vol[1:] / tube_vol[:-1]  # 1/4 per halving
    content_spread = content.max() / content.min()

    # Meter B: |N_delta K| = delta^(3-d), normalized to coarsest delta
    # dim 3 (exp 0): flat, floor delta^eps; dim 5/2 (exp 1/2): steps 2^-1/2 per halving
    dim3_meter = np.ones_like(deltas)  # delta^0
    dim3_floor = (deltas / d0) ** EPS  # c_eps delta^eps
    dim52_meter = (deltas / d0) ** 0.5  # delta^0.5

    ratio_dim3 = dim3_meter[1:] / dim3_meter[:-1]
    ratio_dim52 = dim52_meter[1:] / dim52_meter[:-1]

    # fitted exponent: slope of log|N| vs log delta
    ...
    assert ((growth > 3.2) & (growth < 4.5)).all(), f"#directions must grow ~4x per halving, got {growth}"
    assert np.allclose(vol_ratio, 0.25, atol=1e-12), f"|T|=delta^2 must divide by 4 each halving, got {vol_ratio}"
    assert content_spread < 2.0, f"content #T*|T| must be roughly constant (max/min < 2), got {content_spread:.3f}"
    assert np.allclose(ratio_dim52, 2**-0.5, atol=1e-6), f"dim 5/2 meter must halve-step by 2^-1/2, got {ratio_dim52}"
    assert (ratio_dim3 >= 2 ** (-EPS) - 1e-9).all(), f"dim 3 meter must stay lit (ratio >= 2^-eps), got {ratio_dim3}"
    ...
    math_check(
        ...
        [
            (f"delta={DELTAS[0]}", f"#T={counts[0]:.0f}   |T|=delta^2={tube_vol[0]:.4f}   content={content[0]:.3f}"),
            (
                f"delta={DELTAS[1]}",
                f"#T={counts[1]:.0f}   |T|=delta^2={tube_vol[1]:.4f}   content={content[1]:.3f}   #T x{growth[0]:.2f}",
            ),
            (
                f"delta={DELTAS[2]}",
                f"#T={counts[2]:.0f}   |T|=delta^2={tube_vol[2]:.4f}   content={content[2]:.3f}   #T x{growth[1]:.2f}",
            ),
            (
                f"delta={DELTAS[3]}",
                f"#T={counts[3]:.0f}   |T|=delta^2={tube_vol[3]:.4f}   content={content[3]:.3f}   #T x{growth[2]:.2f}",
            ),
            ...
            (
                "Meter B dim 3 (delta^0)",
                f"ratio/step {np.round(ratio_dim3, 4).tolist()}  (>= 2^-eps={2**-EPS:.4f}, stays lit)",
            ),
            (
                "Meter B dim 5/2 (delta^0.5)",
                f"ratio/step {np.round(ratio_dim52, 4).tolist()}  (= 2^-1/2 = {2**-0.5:.4f}, dims 29%)",
            ),
            ("fitted 3-d exponent", f"dim 3 -> {fit_exp_dim3:.6f} (0),  dim 5/2 -> {fit_exp_dim52:.6f} (0.5)"),
        ],
    )
    ...
    # fixed pool + fixed centers; per stage only radius + count change
    ...
    # union silhouette: length-1 tubes fill ball radius ~1/2
    ...
    def draw_meter_a(upto):
        ...
        axA.set_xlim(-0.5, len(DELTAS) - 0.5)
        axA.set_ylim(0, 1.35)
        axA.set_xticks(stage_x)
        axA.set_xticklabels(xt_labels)
    ...
    def draw_meter_b(upto):
        ...
        xs = stage_x[:upto]
        axB.plot(
            stage_x, dim3_floor, ls="--", lw=1.0, color=COLORS["muted"], label=f"allowed floor delta^eps (eps={EPS:g})"
        )
        axB.plot(xs, dim3_meter[:upto], "-o", color=COLORS["outer"], ms=5, label="dim 3: 3-d = 0  (stays lit)")
        axB.plot(
            xs, dim52_meter[:upto], "-o", color=COLORS["accent"], ms=5, label="dim 5/2: 3-d = 1/2  (dims 29%/step)"
        )
        if upto >= 1:
            axB.plot(stage_x[upto - 1], dim3_meter[upto - 1], "o", color=COLORS["outer"], ms=11, mfc="none", mew=2)
            axB.plot(stage_x[upto - 1], dim52_meter[upto - 1], "o", color=COLORS["accent"], ms=11, mfc="none", mew=2)
        axB.set_xlim(-0.5, len(DELTAS) - 0.5)
        axB.set_ylim(0, 1.15)
        axB.set_xticks(stage_x)
        axB.set_xticklabels(xt_labels)
        axB.set_xlabel("delta")
        axB.set_ylabel("|N_delta K|  (rel.)")
