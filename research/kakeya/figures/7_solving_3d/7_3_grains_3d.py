...
SQRT3 = math.sqrt(3.0)
H = SQRT3 / 2.0  # height of unit-base equilateral triangle
APEX_X = 0.5
...
def _rect(center, angle, length, width) -> Polygon:
    ...
    c = np.asarray(center)
    corners = [c + s1 * (length / 2) * u + s2 * (width / 2) * v for s1, s2 in ((1, 1), (1, -1), (-1, -1), (-1, 1))]
...
def compression_rectangles(n: int, s: float = 0.2):
    ...
    N = len(tris)
    delta = (math.pi / 3.0) / N  # direction separation = tube width
    rects = []
    for t in tris:
        ...
        center = 0.5 * (base_mid + apex)  # centred on needle midpoint
...
def main():
    ...
    # grains disjoint along X -> point in <= 1
    ...
    ax.text(0.5, c / 2, c * 1.35, "fat tube: grains tile it, disjoint", color=COLORS["guide"], fontsize=9, ha="center")
