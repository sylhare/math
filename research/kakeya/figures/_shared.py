"""Shared geometry, validation, and styling helpers for the Kakeya figures.
...
def circle(r: float = 0.5, n: int = 400, cx: float = 0.0, cy: float = 0.0) -> np.ndarray:
    """Circle of radius r centred at (cx, cy) (disc area pi*r^2)."""
    t = np.linspace(0, 2 * math.pi, n, endpoint=False)
    return np.column_stack([cx + r * np.cos(t), cy + r * np.sin(t)])


def triangle_fan_degrees(tri: np.ndarray) -> tuple[float, float]:
    """Direction range (deg, in [0,180)) of segments from base to apex; `tri` rows [baseL, baseR, apex]."""
    (bl, br, ap) = tri
